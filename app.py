...
def connect_to_gsheet():
    # 사용자가 제공한 서비스 계정 정보 (보안상 외부 노출 주의)
    credentials_info = {
...
with st.form("issue_form", clear_on_submit=True):
    # 1. 담당자 선택
    manager = st.radio("담당자 선택", ["이광호", "문정수", "박원덕"], horizontal=True)
        ...
        with st.spinner("데이터를 스프레드시트에 저장 중입니다..."):
            ...
            if sheet:
                try:
                    ...
                    # 선택된 상품 리스트를 쉼표로 구분된 문자열로 변환
                    product_str = ", ".join(products)
                    
                    # 시트에 추가할 데이터 행 구성
                    # 구성: [등록시간, 담당자명, 방문업체명, 상품명, 상세이슈]
